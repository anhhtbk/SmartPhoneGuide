...
class CardTableViewCell: TableViewCell {
  ...
  override func layoutSubviews() {
    ...
    if let imageUrl = d["photo"] as? String,
      let url = URL(string: imageUrl) {
      self.presenterImageView.frame.size.height = Screen.width*0.75
      self.presenterImageView.image = nil
      self.presenterImageView.kf.indicatorType = .activity
      ImageCache.default.retrieveImage(forKey: url.absoluteString, options: nil) { (image, cachType) in
        if let image = image {
          self.presenterImageView.image = image
        }
        else {
          ImageDownloader.default.downloadImage(with: url, retrieveImageTask: nil, options:nil, progressBlock: nil) { (image, error, nil, data) in
            if let image = image?.resize(toWidth: Screen.width) {
              self.presenterImageView.image = image
    ...
    contentLabel.text = d["overview"] as? String
